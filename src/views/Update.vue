...
<script>
    export default {
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.put("http://localhost:8081/books/update",this.ruleForm).then(function (resp) {
                            // console.log(resp)
        ...
        created() {
            var _this = this;
            axios.get("http://localhost:8081/books/findById/"+this.$route.query.id).then(function (resp) {
                // console.log(resp)
